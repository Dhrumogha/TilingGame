...
class TilingGame(object):
    ...
    def score_alphabeta(self, level=None, parent_val=None):
        ...
        board_so_far = tuple(sorted(self.occupied_squares))
        # Return minimax value only when computing it.
        if board_so_far in self.saved_states and 'minimax' in self.saved_states[board_so_far]\
        and level + len(self.occupied_squares)//2 >= max_val:
            return self.saved_states[board_so_far]['minimax']
        ...
        node_order_max = min(2, level)
        node_order_min = 1   # Depth to search for ordering available moves for a MAX & MIN node.
        search_level = node_order_max if self.player == TilingGame.MAX else node_order_min
        scores_available_moves = self.score(level=search_level)[1]    # Score of each available move
        if self.player == TilingGame.MIN:
            max_estimate = min(scores_available_moves)  # Max. possible score at this node
            if max_estimate <= parent_val:  # Prune
                return max_estimate, []
        ...
        for m in available_moves1:
            ...
            if self.player*new_max >= self.player*parent_val:   # Alphabeta prune
                if level >= max_val - 1:    # Save state only if child of root
                    self.update_saved_states(board_so_far,
        ...
        if level + len(self.occupied_squares)//2 >= max_val:
            self.update_saved_states(board_so_far, return_val, 'minimax')

        return return_val

    def update_saved_states(self, board_so_far, return_val, ipstr):
        ...
        for _ in range(num_times):
            self.saved_states[board_so_far][ipstr] = return_val

            # Reflect on Y-axis
            board_refl_y = tuple(sorted((x, n - y-1) for (x, y) in board_so_far))
            self.saved_states[board_refl_y][ipstr] = compute_ret(return_val, ipstr, 'y')

            # Reflect on X-axis.
            board_refl_x = tuple(sorted((m - x-1, y) for (x, y) in board_so_far))
            ...
            board_so_far = tuple(sorted((y, x) for (x, y) in board_so_far))
            if ipstr == 'minimax':
    ...
    def estimate_moves(self):
        '''Return the total estimated number of moves at current position. This is usually an
        overestimate. Return from cache if available, else store the result in cache.'''
        m, n = self.dim
        ...
        if board_so_far in self.saved_states and 'estimate' in self.saved_states[board_so_far]:
            return self.saved_states[board_so_far]['estimate']

        # List of unoccupied squares.
        r_s = [(x, y) for x in range(m) for y in range(n) if (x, y) not in board_so_far]
        size = len(board_so_far) // 2            # no. of dominoes so far.
        while r_s:
            comps = r_s[0]
            T = []
            self.extend_node(comps, r_s, T)            # Determine the component containing comps.
            size += len(T)//2          # (max.) no. of dominoes that can be placed in this component
            r_s = [y for y in r_s if y not in T]    # remove tuples belonging to T from r_s.

        # Save result in cache if required.
        self.update_saved_states(board_so_far, size, 'estimate')
        return size

    def extend_node(self, cn, A, T):
        '''Return the component in A containing cn. The component is collected in the
        parameter T.'''
        T.append(cn)
        # For each potential neighbor of cn
        for x in [(cn[0]-1, cn[1]), (cn[0]+1, cn[1]), (cn[0], cn[1]-1), (cn[0], cn[1]+1)]:
            if x in A and x not in T:
                self.extend_node(x, A, T)
